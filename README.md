...
* CSS 中类 (classes) 和 ID 的区别。
  一：表现形式上，ID权重大于class，也就是说ID的优先级高于Class，ID权重为100，class为1.
  二：class为通用属性，可以给多个标签使用，ID是唯一的。Web标准中是不允许出现多个ID一样的，为了兼容不同的浏览器，尽量不要使用多个相同的ID。
* 请问 "resetting" 和 "normalizing" CSS 之间的区别？你会如何选择，为什么？
  resetting无前提重置所有样式，normalizing有条件的重置部分样式。
* 请解释浮动 (Floats) 及其工作原理。
...
* 请说明javascript下如何传递中文？
  答:javascript传递中文需要对中文做转码处理，如果是js传递到js，可以对字符串做两次转码处理：
   var str = '我是中文';
   str = encodeURI(encodeURI(str));
   javascript接收端处理时用decodeURI(decodeURI(getQueryString('str')));即可。
* 请解释事件代理 (event delegation)。
  事件代理是根据事件冒泡的原理，将事件出发的原始节点的行为委托给父辈元素去处理。
...
* 描述以下变量的区别：`null`，`undefined` 或 `undeclared`？
  null是指空值，typeof(null)返回的是一个Object，而undefined是window对象的一个属性，定义任意一个变量，但是没有初始化的话，这个变量的typeof就是undefined，且这个变量是指向window.undefined属性的引用。
  undecared是一个js语法错误。
  * 该如何检测它们？
